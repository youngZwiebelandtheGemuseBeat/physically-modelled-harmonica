...
def opening_area_clipped(displacement_m: float, reed: ReedParameters) -> float:
    """Opening area A_i(x_i) = W_i max(0, h_i0 + alpha_i x_i)."""

    return reed.slot_width_m * max(0.0, reed_gap(displacement_m, reed))


def opening_area_symmetric_smooth(displacement_m: float, reed: ReedParameters) -> float:
    # Experimental effective-area law: approximates reopening on the opposite side
    # of the reed plate. This does not model contact, leakage, side-dependent
    # pressure loading or local slot losses.
    z = reed_gap(displacement_m, reed)
    eps = 5.0e-6
    return reed.slot_width_m * (sqrt(z * z + eps * eps) - eps)


def opening_area_asymmetric_reopen(
    displacement_m: float,
    reed: ReedParameters,
    negative_gain: float = 0.25,
    closed_band_m: float = 5.0e-6,
) -> float:
    # Experimental reduced inward-reopening law. The negative side is weighted by
    # negative_gain and delayed by closed_band_m. This does not model contact,
    # leakage, side-dependent pressure loading or local slot losses.
    if negative_gain < 0.0:
        raise ValueError("negative_gain must be >= 0.0")
    if closed_band_m < 0.0:
        raise ValueError("closed_band_m must be >= 0.0")
    z = reed_gap(displacement_m, reed)
    return reed.slot_width_m * (
        max(0.0, z) + negative_gain * max(0.0, -z - closed_band_m)
    )
...
def opening_area(
    ...
    """Select the effective opening-area law."""

    if opening_model == "clipped":
        return opening_area_clipped(displacement_m, reed)
    if opening_model == "symmetric_smooth":
        return opening_area_symmetric_smooth(displacement_m, reed)
    if opening_model == "asymmetric_reopen":
        return opening_area_asymmetric_reopen(
            displacement_m,
            reed,
            negative_gain,
            closed_band_m,
        )
    raise ValueError(f"unknown opening model: {opening_model!r}")
...
def draw_reed_force(p_c_pa: float, params: ModelParameters) -> float:
    """Draw reed opens for positive and closes for negative chamber pressure."""
