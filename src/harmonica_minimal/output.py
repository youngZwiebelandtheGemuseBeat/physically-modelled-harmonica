...
from .analysis import select_steady_state_window
...
class OscillationMeasurement:
    """Clearly defined late-window values for paper comparisons."""

    name: str
    unit: str
    mean: float
    ac_rms: float
    half_peak_to_peak: float
...
def harmonic_ratios(signal: np.ndarray, sample_rate_hz: int, f0_hz: float, count: int = 10) -> list[float]:
    """Return local-band H1-H10 magnitude ratios relative to H1."""
    ...
    magnitudes = np.abs(np.fft.rfft(centered * window))
    freqs = np.fft.rfftfreq(centered.size, 1.0 / sample_rate_hz)
    bin_width_hz = float(freqs[1] - freqs[0]) if freqs.size > 1 else 0.0
    search_half_width_hz = max(2.0 * bin_width_hz, 0.01 * f0_hz)
    values = []
    for harmonic in range(1, count + 1):
        ...
        if target > freqs[-1]:
            values.append(0.0)
            continue
        band = np.abs(freqs - target) <= search_half_width_hz
        values.append(float(np.sqrt(np.sum(magnitudes[band] ** 2))) if np.any(band) else 0.0)
...
def oscillation_measurements(result: SimulationResult) -> list[OscillationMeasurement]:
    """Measure pressure and reed motion over the shared late analysis window."""

    start, stop = _steady_region(result)
    signals = (
        ("chamber pressure p_c", "Pa", result.p_c),
        ("blow reed displacement x_b", "m", result.x_b),
        ("draw reed displacement x_d", "m", result.x_d),
    )
    measurements = []
    for name, unit, values in signals:
        window = np.asarray(values[start:stop], dtype=float)
        if window.size == 0:
            measurements.append(OscillationMeasurement(name, unit, 0.0, 0.0, 0.0))
            continue
        ...
        measurements.append(
            OscillationMeasurement(
                ...
                unit=unit,
                mean=mean,
                ac_rms=float(np.sqrt(np.mean((window - mean) ** 2))),
                half_peak_to_peak=0.5 * float(np.max(window) - np.min(window)),
            )
        )
    return measurements
...
def diagnostics_text(
    ...
    oscillations = oscillation_measurements(result)
    ...
    lines = [
        ...
        f"prescribed mouth-pressure boundary: {result.params.mouth_pressure_pa:.6g} Pa",
        f"opening model: {result.opening_model}",
        f"opening negative gain: {result.opening_negative_gain:.6g}",
        f"opening closed band: {result.opening_closed_band_m * 1.0e6:.6g} um",
        f"active pressure-area scale: {result.active_pressure_area_scale:.6g}",
        f"active damping scale: {result.active_damping_scale:.6g}",
        f"active discharge scale: {result.active_discharge_scale:.6g}",
        f"passive pressure-area scale: {result.passive_pressure_area_scale:.6g}",
        f"passive damping scale: {result.passive_damping_scale:.6g}",
        f"passive discharge scale: {result.passive_discharge_scale:.6g}",
        ...
        "",
        "Steady-state oscillation measurements with explicit definitions:",
    ]
    for measurement in oscillations:
        lines.append(
            f"- {measurement.name} [{measurement.unit}]: mean={measurement.mean:.6g}, "
            f"AC RMS={measurement.ac_rms:.6g}, "
            f"half peak-to-peak={measurement.half_peak_to_peak:.6g}"
        )
    lines.extend([
        "",
