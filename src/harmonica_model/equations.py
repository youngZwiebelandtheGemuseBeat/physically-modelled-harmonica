...
def reed_opening_area(displacement_m: float, reed: ReedParams) -> float:
    """Effective slot area A_i(x_i), clipped at zero for a closed opening."""

    gap_m = reed.rest_opening_m + reed.displacement_to_gap * displacement_m
    if gap_m <= 0.0:
        return 0.0
    return reed.slot_width_m * gap_m
...
def derived_values(t_s: float, state: np.ndarray, params: ModelParams) -> DerivedValues:
    ...
    dp_c = chamber_pressure_derivative(q_b, q_d, params)
    audio_pressure = params.pressure_output_gain * p_t + params.acoustic_flow_gain_pa_s_m3 * (q_b - q_d)
